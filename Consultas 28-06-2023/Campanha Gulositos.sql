----------------------CTE PARA CONSULTAR O FATURAMENTO---------------------------
WITH Fat AS 
    (SELECT ped.codusur,
    ...
    WHERE prod.codfornec = 1719
        AND ped.data BETWEEN '26-jun-2023' and '31-jul-2023'
        AND ped.posicao NOT LIKE 'C'
        AND ped.vlbonific = 0
    Group By ped.codusur),
----------------------CTE PARA CONSULTAR A POSITIVAÇÃO--------------------------- 
    DN AS
        (SELECT ped.codusur,
...
WHERE usur.codusur in (140, 141, 142, 143, 145, 148, 150, 151, 152, 153, 155, 
                        156, 157, 158, 161, 164, 167, 168, 169, 170, 172, 174) 
